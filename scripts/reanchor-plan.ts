...
import { generateSeedSql, sqlJson, sqlNum, sqlStr } from "./generate-supabase-seed";
 ...
 */
export function buildReanchorUpdateSql(reanchored: Seed): string {
  ...
  for (const session of reanchored.week) {
    ...
    statements.push(
      `update public.planned_sessions set date = ${sqlStr(row.date)} where id = ${sqlStr(row.id)} and user_id = ${sqlStr(TEST_USER_ID)};`
    );
  }

  statements.push(
    "",
    `update public.blocks set week = ${sqlNum(reanchored.block.week)} where user_id = ${sqlStr(TEST_USER_ID)};`,
    ""
  );
  ...
  for (const proposal of reanchored.proposals) {
    ...
    statements.push(
      `update public.proposals set payload = ${sqlJson(row.payload)} where id = ${sqlStr(row.id)} and user_id = ${sqlStr(TEST_USER_ID)};`
    );
  }
...
export function runReanchor(options: RunReanchorOptions): RunReanchorResult {
  ...
  const updateSql = buildReanchorUpdateSql(reanchored);
